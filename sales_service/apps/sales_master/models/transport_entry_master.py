from django.db import models

from shared.base_models import BaseMaster

from apps.sales_master.utils.random_code_gen import generate_random_no, generate_random_sc


class TransportEntryMaster(BaseMaster):

    class TransportType(models.TextChoices):
        CREDITOR = "creditor", "Creditor"
        DEBITOR = "debitor", "Debitor"

    class PaymentType(models.TextChoices):
        CREDIT = "credit", "Credit"
        DEBIT = "debit", "Debit"

    # -------------------------
    # Transport Details
    # -------------------------
    transport_date = models.DateField()
    transport_type = models.CharField(max_length=10, choices=TransportType.choices)
    transport_name = models.CharField(max_length=150)
    contact_person = models.CharField(max_length=150)
    phone_no = models.CharField(max_length=20, blank=True, null=True)
    mobile_no = models.CharField(max_length=20)
    other_mobile_1 = models.CharField(max_length=20, blank=True, null=True)
    other_mobile_2 = models.CharField(max_length=20, blank=True, null=True)
    other_mobile_3 = models.CharField(max_length=20, blank=True, null=True)
    whatsapp_no = models.CharField(max_length=20, blank=True, null=True)

    # Master Service Country/State/District/City `unique_id`s (no DB-level
    # FK - see SALES_SERVICE_README.md).
    country_id = models.UUIDField()
    state_id = models.UUIDField()
    district_id = models.UUIDField()
    city_id = models.UUIDField()

    building_no = models.CharField(max_length=50)
    street = models.CharField(max_length=150)
    area = models.CharField(max_length=150)
    pincode = models.CharField(max_length=10)

    has_gst = models.BooleanField(default=False)
    gst_no = models.CharField(max_length=20, blank=True, null=True)

    to_email = models.EmailField(blank=True, null=True)
    cc_mail = models.CharField(max_length=255, blank=True, null=True)

    opening_balance = models.DecimalField(max_digits=15, decimal_places=2, default=0)
    payment_type = models.CharField(max_length=10, choices=PaymentType.choices)

    # Plain list of Master Service Site `unique_id`s (no DB-level M2M).
    sites = models.JSONField(default=list, blank=True)

    target_limit = models.DecimalField(max_digits=15, decimal_places=2)

    # -------------------------
    # Bank Details
    # -------------------------
    bank_name = models.CharField(max_length=150, blank=True, null=True)
    branch = models.CharField(max_length=150, blank=True, null=True)
    account_no = models.CharField(max_length=50, blank=True, null=True)
    ifsc_code = models.CharField(max_length=20, blank=True, null=True)
    pan_no = models.CharField(max_length=20, blank=True, null=True)

    tds = models.BooleanField(default=False)
    tds_document = models.FileField(upload_to="transport/tds/", blank=True, null=True)

    # -------------------------
    # System Generated (never user-supplied)
    # -------------------------
    random_no = models.CharField(max_length=5, unique=True, blank=True)
    random_sc = models.CharField(max_length=12, unique=True, blank=True)

    class Meta:
        ordering = ["-created_at"]

    def __str__(self):
        return self.transport_name

    def save(self, *args, **kwargs):
        if not self.random_no:
            self.random_no = generate_random_no(TransportEntryMaster)
        if not self.random_sc:
            self.random_sc = generate_random_sc(TransportEntryMaster)
        super().save(*args, **kwargs)

    def delete(self, *args, **kwargs):
        """
        Soft delete
        """
        self.is_deleted = True
        self.is_active = False
        self.save(update_fields=["is_deleted", "is_active"])
