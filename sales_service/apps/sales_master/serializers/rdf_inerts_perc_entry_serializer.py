...
class RdfInertsPercEntrySerializer(serializers.ModelSerializer):
    """
    `site_name` is a plain string (Sales Service has no local copy of Master
    Service's Site data - see SALES_SERVICE_README.md). The client resolves
    the site name from Master Service's own site API and submits it directly,
    rather than a Site `unique_id` that this service could look up.
    """
