...
class ScrapQuotationComparison(models.Model):
    ...
    def __str__(self):
        return f"ScrapComp-{self.quote_month}-{self.site_id}"
...
class ScrapQuotationComparisonSub(models.Model):
    unique_id = models.UUIDField(default=uuid.uuid4, editable=False, unique=True)
    main = models.ForeignKey(ScrapQuotationComparison, on_delete=models.CASCADE, related_name="sub_items")

    site_id = models.UUIDField(null=True, blank=True)
    quote_entry_no = models.CharField(max_length=50, blank=True, default="")
