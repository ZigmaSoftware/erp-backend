...
class ScrapQuotationMain(models.Model):
...
class ScrapQuotationSub(models.Model):
    unique_id = models.UUIDField(default=uuid.uuid4, editable=False, unique=True)
    main = models.ForeignKey(ScrapQuotationMain, on_delete=models.CASCADE, related_name="sub_items")

    item_id = models.UUIDField(help_text="ItemCreation unique_id")
    material_type = models.CharField(max_length=50, blank=True, default="Scrap")
    ...
    def save(self, *args, **kwargs):
        self.tot_amount = self.amount + (self.amount * self.gst_value / 100) if self.gst_value else self.amount
