...
SALES_SERVICE_BASE = "http://127.0.0.1:8003"
...
class SalesServiceProxy(View):
    def dispatch(self, request, *args, **kwargs):
        path = request.path.replace("/api/sales-service/", "")
        url = f"{SALES_SERVICE_BASE}/{path}"
        ...
        try:
            ...
            logger.exception("Sales service unreachable")
            return JsonResponse({"detail": "Service unavailable"}, status=503)
